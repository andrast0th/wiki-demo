...
Every VirtuCare clinic and provider gets a shareable booking link where patients can view availability and self-schedule, without needing a VirtuCare account.

## Finding your link

- **Clinic-wide link:** `https://book.virtucare.example/riverside-family-health`
- **Provider-specific link:** `https://book.virtucare.example/riverside-family-health/dr-osei`
...
```html
<iframe
  src="https://book.virtucare.example/riverside-family-health?embed=true"
  width="100%"
  height="700"
  frameborder="0">
</iframe>
...
The `embed=true` parameter removes VirtuCare's header/footer chrome so the widget blends into your site.
...
3. Enter their name, date of birth, email, and phone number, or log into an existing VirtuCare patient account.
...
The booking flow automatically detects returning patients by matching email + date of birth against existing records, and pre-fills their information rather than asking them to re-enter it. If a match can't be made confidently, VirtuCare creates a new patient record and flags it for front-desk review to check for accidental duplicates.

## Restricting who can book

Under **Schedule → Booking Links → Restrictions**, you can require:

- **State restriction** — only allow booking from patients physically located in states where the provider is licensed (uses the patient's stated address; VirtuCare does not geofence by IP for this).
