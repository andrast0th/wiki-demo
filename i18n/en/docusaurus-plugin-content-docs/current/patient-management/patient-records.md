...
Every patient in DemoMed has a chart containing demographics, visit history, clinical notes, intake responses, and attached documents.
...
- Count toward the mandatory documentation your state's telehealth regulations may require to be finalized within a set number of days of the visit — DemoMed shows a reminder banner on any draft note older than 5 days.

## Merging duplicate patients

If two records are created for the same person (e.g. a returning patient who booked with a slightly different email), an **Owner** or **Provider** can merge them from **Patients → [patient] → Merge Duplicate**. Merging combines visit history, forms, and documents into a single chart and is **not reversible** — DemoMed shows a side-by-side comparison and requires typing "MERGE" to confirm.
