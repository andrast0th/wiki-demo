...
DemoMed video visits run directly in the browser (Chrome, Edge, Safari 16+, or Firefox) using WebRTC — no separate app install is required for providers or patients.
...
Before your first consultation, run DemoMed's built-in device check:
...
Set default camera, microphone, and speaker under **Settings → Video → Default Devices**. This is especially useful if you use different equipment at different locations (e.g. a laptop mic at home, a USB headset at the office) — DemoMed will remember the last device you selected per browser profile, but setting explicit defaults avoids accidentally starting a visit on the wrong microphone.
