...
Clinical and billing retention periods are set conservatively to satisfy the longest common state and federal requirements across the jurisdictions DemoMed operates in — check your own state's medical records retention law if your patients are concentrated in a single state, as some require longer minimums (DemoMed's defaults will never fall below your state's floor).

## Requesting patient record deletion

Patients occasionally ask to have their data deleted. Because clinical records are subject to mandatory retention requirements, DemoMed **cannot** delete an active patient chart on request in the way a typical consumer app might delete an account. Instead:
