...
Your clinic profile controls what patients see when they receive appointment confirmations, visit reminder links, and invoices from DemoMed.
...
- A custom "from" name for email notifications (e.g. "Riverside Family Health" instead of the default "DemoMed Notifications").

:::info
Custom email sending domains (so reminders come from `@riversidefamilyhealth.com` instead of `@demomed.example`) require the **Practice** plan or higher and DNS verification. See [Custom notification domains](/faq/technical-faq#custom-domains).
:::
