...
DemoMed supports both insurance billing (via claims clearinghouse integration) and direct self-pay, and clinics can offer either or both.

## Connecting a clearinghouse

Under **Billing → Insurance → Clearinghouse**, connect your clearinghouse account (DemoMed supports the major national clearinghouses via a standard EDI 837 connection). You'll need your clinic's:
...
To collect card payments (co-pays, self-pay balances, or no-show fees), connect a payment processor under **Billing → Payment Settings → Payment Processor**. DemoMed does not store full card numbers — the processor tokenizes card details and DemoMed only stores the token and card last-4 for reference, in line with PCI DSS requirements.

## Collecting co-pays

If a patient's insurance plan has a known co-pay amount on file, DemoMed can prompt for payment either:
