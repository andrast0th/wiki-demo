...
Every DemoMed clinic and provider gets a shareable booking link where patients can view availability and self-schedule, without needing a DemoMed account.

## Finding your link

- **Clinic-wide link:** `https://book.demomed.example/riverside-family-health`
- **Provider-specific link:** `https://book.demomed.example/riverside-family-health/dr-osei`
...
The `embed=true` parameter removes DemoMed's header/footer chrome so the widget blends into your site.
...
3. Enter their name, date of birth, email, and phone number, or log into an existing DemoMed patient account.
...
The booking flow automatically detects returning patients by matching email + date of birth against existing records, and pre-fills their information rather than asking them to re-enter it. If a match can't be made confidently, DemoMed creates a new patient record and flags it for front-desk review to check for accidental duplicates.

## Restricting who can book

Under **Schedule → Booking Links → Restrictions**, you can require:

- **State restriction** — only allow booking from patients physically located in states where the provider is licensed (uses the patient's stated address; DemoMed does not geofence by IP for this).
