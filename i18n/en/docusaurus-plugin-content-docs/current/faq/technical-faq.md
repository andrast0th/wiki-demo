...
- **Safari:** Safari menu → Settings → Websites → Camera/Microphone → find the DemoMed domain → set to "Allow."
...
This is almost always a bandwidth issue rather than a DemoMed outage. Try, in order:
...
If problems persist across multiple visits from the same location, it may be worth having your office internet connection assessed — DemoMed's minimum bandwidth recommendations assume no other heavy usage sharing the same connection during visits.

### Can I use DemoMed with a second monitor?

Yes — nothing in DemoMed restricts multi-monitor setups. A common configuration is the video visit on one screen and the patient's chart/clinical notes on the other, opened as separate browser windows.
...
Patients can join over cellular data if Wi-Fi isn't available; DemoMed's Low Bandwidth Mode (see above) helps significantly on constrained mobile connections. There is currently no audio-only "dial-in by phone" fallback — if a patient truly cannot get a working video connection, the visit will need to be converted to a phone call outside DemoMed or rescheduled.
