...
Fiecare clinică și fiecare medic din SuperMed primește un link de rezervare care poate fi distribuit, unde pacienții pot vedea disponibilitatea și se pot programa singuri, fără să aibă nevoie de un cont SuperMed.

## Găsirea linkului tău

- **Link general al clinicii:** `https://book.supermed.example/riverside-family-health`
- **Link specific medicului:** `https://book.supermed.example/riverside-family-health/dr-osei`
...
Parametrul `embed=true` elimină antetul/subsolul SuperMed, astfel încât widget-ul se integrează vizual în site-ul tău.
...
3. Introduc numele, data nașterii, emailul și numărul de telefon, sau se autentifică într-un cont de pacient SuperMed existent.
...
Fluxul de rezervare detectează automat pacienții existenți, potrivind emailul și data nașterii cu fișele deja existente, și le pre-completează informațiile în loc să le ceară să le reintroducă. Dacă nu se poate face o potrivire sigură, SuperMed creează o fișă nouă de pacient și o marchează pentru verificare de către recepție, pentru a depista eventuale duplicate accidentale.

## Restricționarea cine se poate programa

Din **Agendă → Linkuri de rezervare → Restricții**, poți impune:

- **Restricție de stat/regiune** — permite rezervarea doar pacienților aflați fizic în regiunile în care medicul este licențiat (folosește adresa declarată de pacient; SuperMed nu face geo-blocare după IP pentru această funcție).
